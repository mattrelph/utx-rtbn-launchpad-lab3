...
/*
struct periodicTCB{
	void (*periodFunction)(void);	//Pointer to periodic function
	uint32_t period;	//time interval to repeat function
};

typedef struct periodicTCB periodicTCBType;
periodicTCBType ptcbs[NUMPERIODIC];
uint32_t Counter = 1;
uint32_t MaxCounter = 1;*/
...
// initialize 6 stacks, including initial PC
	SetInitialStack(0); 
